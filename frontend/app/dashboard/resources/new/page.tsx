...
export default function NewResourcePage() {
    ...
    return (
        <div className="max-w-3xl mx-auto relative">
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full bg-[#375BD2]/5 blur-[120px] rounded-full pointer-events-none -z-10" />
            ...
            <div className="bg-[#0a0a0f]/60 backdrop-blur-2xl rounded-3xl border border-white/10 p-8 md:p-10 shadow-2xl relative overflow-hidden">
                <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-white/20 to-transparent" />

                <h2 className="text-3xl font-bold text-[#F4F4F5] mb-2 text-center">Add Resource</h2>
                <p className="text-gray-500 text-center mb-10">Create a new data asset on Ethereum Sepolia</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {[
                                { value: 'IMAGE', icon: Image, label: 'Image', desc: 'Upload digital assets' },
                                ...
                                <button
                                    ...
                                    className={`relative flex flex-col items-center gap-3 p-6 rounded-2xl border transition-all duration-300 group ${type === value
                                        ? 'bg-white/10 border-[#375BD2] shadow-[0_0_30px_-10px_rgba(55,91,210,0.3)]'
                                        : 'bg-white/5 border-white/5 hover:bg-white/10 hover:border-white/10'
                                        }`}
                                >
                                    {type === value && (
                                        <div className="absolute top-3 right-3 text-[#375BD2]">
                                            <Check size={16} strokeWidth={3} />
                                        </div>
                                    )}
                                    <div className={`p-3 rounded-xl transition-colors ${type === value ? 'bg-[#375BD2] text-white' : 'bg-black/50 text-gray-400 group-hover:text-[#F4F4F5]'}`}>
                                        <Icon size={24} />
                                    </div>
                    ...
                    <div className="space-y-6 bg-black/20 p-6 rounded-2xl border border-white/5">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                placeholder="e.g., Premium API Dataset"
                                className="w-full px-5 py-4 bg-black/50 border border-white/10 rounded-xl text-[#F4F4F5] placeholder-gray-600 focus:outline-none focus:border-[#375BD2] focus:ring-1 focus:ring-[#375BD2] transition-all"
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                rows={3}
                                className="w-full px-5 py-4 bg-black/50 border border-white/10 rounded-xl text-[#F4F4F5] placeholder-gray-600 focus:outline-none focus:border-[#375BD2] focus:ring-1 focus:ring-[#375BD2] transition-all resize-none"
                            />
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    min="1"
                                    step="1"
                                    className="w-full px-5 py-4 bg-black/50 border border-white/10 rounded-xl text-[#F4F4F5] placeholder-gray-600 focus:outline-none focus:border-[#375BD2] transition-all"
                                />
                            </div>
                        </div>
                        ...
                        {type === 'IMAGE' && (
                            <div>
                                ...
                                <div className="border border-dashed border-white/20 rounded-2xl p-8 text-center hover:border-[#375BD2]/50 hover:bg-white/5 transition-all group cursor-pointer relative overflow-hidden">
                                    {imagePreview ? (
                                        <div className="relative inline-block">
                                            ...
                                            <button
                                                ...
                                                className="absolute -top-3 -right-3 p-2 bg-[#dc2626] text-white rounded-full hover:scale-110 transition-transform shadow-lg"
                                            >
                                                ✕
                                            </button>
                                        </div>
                                        ...
                                        <label className="cursor-pointer block">
                                            <div className="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform">
                                                <Upload size={32} className="text-gray-400 group-hover:text-[#375BD2] transition-colors" />
                                            </div>
                    ...
                    {/* Error */}
                    {error && (
                        <div className="bg-[#dc2626]/10 border border-[#dc2626]/30 text-[#ef4444] p-4 rounded-xl text-sm flex items-center gap-2">
                            ⚠️ {error}
                        </div>
                    )}
                    ...
                    <button
                        ...
                        className="w-full py-4 bg-gradient-to-r from-[#375BD2] to-[#2B4ABF] hover:opacity-90 rounded-xl font-bold text-white text-lg shadow-lg shadow-blue-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 group"
                    >
                        {isSubmitting ? (
                            <>

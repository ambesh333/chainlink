...
function httpGet(runtime2, http, url) {
...
function httpPost(runtime2, http, url, body) {
  const response = http.sendRequest(runtime2, {
    request: {
      url,
      method: "POST",
...
function executeNode(runtime2, http, node, context) {
  const config = runtime2.config;
  const { blockType, config: nodeConfig } = node.data;
  runtime2.log(`  Executing node: ${node.data.label} (${blockType})`);
  switch (node.type) {
    case "trigger": {
    ...
    case "data": {
      if (blockType === "fetch_stats") {
        ...
        const stats = httpGet(runtime2, http, `${config.backendUrl}/cre/resource-stats/${resourceId}`);
        runtime2.log(`  Stats: price=${stats.currentPrice}, access=${stats.accessCount}, earned=${stats.totalEarnings}`);
    ...
    case "condition": {
      if (blockType === "compare") {
        const { metric, operator, value: value2 } = nodeConfig;
        const metricValue = context[metric];
        if (metricValue === undefined) {
          runtime2.log(`  Condition: metric "${metric}" not found in context`);
          return { success: true, output: { result: false }, conditionResult: false };
        }
        let result = false;
        switch (operator) {
          case ">":
            result = metricValue > value2;
            break;
          case "<":
            result = metricValue < value2;
            break;
        ...
        runtime2.log(`  Condition: ${metric}(${metricValue}) ${operator} ${value2} → ${result}`);
        return { success: true, output: { metric, metricValue, operator, value: value2, result }, conditionResult: result };
      }
    ...
    case "ai": {
      if (blockType === "price_analysis") {
        ...
        try {
          const analysis = httpPost(runtime2, http, `${config.backendUrl}/cre/ai-price-analysis`, analysisInput);
          if (analysis.error) {
            runtime2.log(`  AI Analysis failed: ${analysis.error}. Using fallback.`);
            const fallback = {
          ...
          runtime2.log(`  AI Analysis: action=${analysis.action}, recommended=${analysis.recommendedPrice}, confidence=${analysis.confidence}`);
          context.aiAnalysis = analysis;
          context.recommendedPrice = analysis.recommendedPrice;
          return { success: true, output: analysis };
        } catch (e) {
          runtime2.log(`  AI Analysis exception: ${e.message || "unknown"}. Using fallback.`);
          const fallback = {
            recommendedPrice: context.currentPrice || 0,
            changePercent: 0,
            reasoning: "AI analysis unavailable — holding current price",
    ...
    case "action": {
      if (blockType === "stop") {
        runtime2.log(`  Workflow completed`);
      ...
      if (blockType === "update_price") {
        ...
        newPrice = Math.round(newPrice * 1e6) / 1e6;
        try {
          const result = httpPost(runtime2, http, `${config.backendUrl}/cre/workflow-action`, {
            action: "update_price",
            resourceId,
            value: newPrice
          });
          runtime2.log(`  Action: Updated price → ${newPrice} ETH`);
          return { success: true, output: result };
        } catch (e) {
          runtime2.log(`  Action failed: ${e.message || "unknown"}`);
      ...
      if (blockType === "toggle_resource") {
        ...
        try {
          ...
          runtime2.log(`  Action: Toggled resource → ${nodeConfig.active ? "active" : "inactive"}`);
          return { success: true, output: result };
        } catch (e) {
          runtime2.log(`  Action failed: ${e.message || "unknown"}`);
...
function executeWorkflow(runtime2, http, workflow2) {
  const definition = workflow2.definition;
  const nodes = definition.nodes || [];
  const edges = definition.edges || [];
  runtime2.log(`
=== Executing workflow: ${workflow2.name} (${workflow2.id}) ===`);
  if (nodes.length === 0) {
  ...
  while (queue.length > 0) {
    ...
    if (!node)
      continue;
    stepsExecuted++;
    const result = executeNode(runtime2, http, node, context);
    logLines.push(`[${node.data.label}] ${result.success ? "OK" : "FAIL"}: ${JSON.stringify(result.output).substring(0, 200)}`);
    if (!result.success) {
      return {
        status: "failed",
        log: logLines.join(`
...
var onCronTrigger = (runtime2) => {
  ...
  runtime2.log(`Workflow engine triggered at ${now.toISOString()}`);
  const http = new ClientCapability2;
  let workflows = [];
  try {
    const response = httpGet(runtime2, http, `${config.backendUrl}/cre/active-workflows`);
    workflows = response.workflows || [];
    runtime2.log(`Found ${workflows.length} active workflow(s)`);
  } catch {
    runtime2.log("Failed to fetch active workflows");
    return "Error: failed to fetch active workflows";
  }
  if (workflows.length === 0) {
    runtime2.log("No active workflows to execute");
    return "No active workflows";
  }
  const evmClient = new ClientCapability(ClientCapability.SUPPORTED_CHAIN_SELECTORS["ethereum-testnet-sepolia"]);
  let totalExecuted = 0;
  let totalSucceeded = 0;
  const executionSummaries = [];
  for (const workflow2 of workflows) {
    try {
      const result = executeWorkflow(runtime2, http, workflow2);
      httpPost(runtime2, http, `${config.backendUrl}/cre/workflow-execution`, {
      ...
      if (result.status === "completed")
        totalSucceeded++;
      executionSummaries.push(`${workflow2.name}: ${result.status} (${result.steps} steps)`);
      runtime2.log(`Workflow ${workflow2.name}: ${result.status}`);
    } catch (err) {
      runtime2.log(`Workflow ${workflow2.name} failed with error`);
      executionSummaries.push(`${workflow2.name}: error`);
      totalExecuted++;
      try {
        httpPost(runtime2, http, `${config.backendUrl}/cre/workflow-execution`, {
          workflowId: workflow2.id,
  ...
  const auditData = encodeAbiParameters([{ type: "uint256" }, { type: "uint256" }, { type: "uint256" }], [
    BigInt(totalExecuted),
    BigInt(totalSucceeded),
    BigInt(Math.floor(now.getTime() / 1000))
  ]);
  runtime2.log(`Generating audit report: ${totalSucceeded}/${totalExecuted} workflows succeeded`);
  ...
  runtime2.log("Submitting audit report on-chain...");
  ...
  runtime2.log(`Audit report tx: ${auditTxHash}`);
  runtime2.log(`View: https://sepolia.etherscan.io/tx/${auditTxHash}`);

...
var onCronTrigger = (runtime2) => {
  ...
  runtime2.log(`Expiry watchdog triggered at ${now.toISOString()}`);
  ...
  try {
    ...
    expiredKeys = (result.expired ?? []).map((e) => e.escrowKey);
    runtime2.log(`Found ${expiredKeys.length} expired escrow(s)`);
  } catch {
    runtime2.log("Failed to parse expired-escrows response");
    return "Error: failed to fetch expired escrows";
  }
  if (expiredKeys.length === 0) {
    runtime2.log("No expired escrows found");
  ...
  for (const escrowKey of expiredKeys) {
    runtime2.log(`Processing expired escrow: ${escrowKey}`);
    const reportData = encodeAbiParameters([{ type: "bytes32" }, { type: "bool" }], [escrowKey, false]);
    runtime2.log(`Generating signed report → escrowKey=${escrowKey}, payMerchant=false (refund)`);
    ...
    runtime2.log(`Submitting expiry report to DisputeConsumer...`);
    ...
    runtime2.log(`Expiry refund tx succeeded: ${finalTxHash}`);

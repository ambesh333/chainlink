...
var SETTLEMENT_REQUESTED_EVENT_SIG = "0x96186b8375a7f1e5d882fb44d498e7e41e518e3ae009fd917965bffc28b3b65e";
function decodeTopic(topic) {
  ...
  if (topic instanceof Uint8Array) {
    return "0x" + Buffer.from(topic).toString("hex");
  }
  throw new Error("Invalid topic format");
}
var onSettlementRequested = (runtime2, triggerOutput) => {
  ...
  runtime2.log(`Escrow Key: ${escrowKey}`);
  runtime2.log(`Original TX: ${txHash}`);
  const confidentialHttp = new ClientCapability2;
  const verifyResponse = confidentialHttp.sendRequest(runtime2, {
    request: {
  ...
  try {
    ...
    runtime2.log(`Delivery verification: delivered=${delivered}`);
  } catch {
    runtime2.log("Failed to parse delivery verification response");
  }
  if (!delivered) {
    runtime2.log(`Delivery not verified for key=${escrowKey}. Skipping auto-settlement.`);
    return `Skipped: ${escrowKey} — delivery not verified`;
  }
  const reportData = encodeAbiParameters([{ type: "bytes32" }, { type: "bool" }], [escrowKey, true]);
  runtime2.log(`Generating signed report → escrowKey=${escrowKey}, payMerchant=true`);
  ...
  runtime2.log(`Submitting report to DisputeConsumer...`);

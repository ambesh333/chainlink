...
var onDisputeRaised = (runtime2, triggerOutput) => {
  ...
  runtime2.log(`DisputeRaised → key=${escrowKey}`);
  ...
  try {
    ...
    const context = JSON.parse(bodyStr);
    runtime2.log(`Backend context loaded`);
    if (context.autoApprove === true) {
      payMerchant = true;
    }
  } catch {
    runtime2.log("Backend fetch failed → default refund agent");
  }
  const reportPayload = encodeAbiParameters([{ type: "bytes32" }, { type: "bool" }], [escrowKey, payMerchant]);
  ...
  runtime2.log(`Settlement submitted → key=${escrowKey}, payMerchant=${payMerchant}`);
  return `Resolved ${escrowKey} → payMerchant=${payMerchant}`;
};
